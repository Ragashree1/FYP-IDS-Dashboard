...
from services.event_service import fetch_cicflow_logs_from_es, export_cicflow_logs_to_pkl, get_logs_from_db
...
@router.get("/events")
def get_events(orgId: int = Query(...), db: Session = Depends(get_db)):
    """Get network traffic events from the DB"""
    return get_logs_from_db(orgId, db)
