...
# Track retransmissions/drops (simplified approach)
retransmission_tracker = defaultdict(set)
...
for pkt in packets:
    if pkt.haslayer(IP) and pkt.haslayer(TCP):
        ...
        tcp_seq = pkt[TCP].seq
        tcp_ack = pkt[TCP].ack
        # Simplified retransmission detection (duplicate seq for the same IP/port pair)
        if (tcp_seq, ip_src, tcp_src_port, ip_dst, tcp_dst_port) in retransmission_tracker:
            flows[flow_id]["sloss"] += 1
        else:
            retransmission_tracker[(tcp_seq, ip_src, tcp_src_port, ip_dst, tcp_dst_port)] = True

        if (tcp_seq, ip_dst, tcp_dst_port, ip_src, tcp_src_port) in retransmission_tracker:
            flows[flow_id]["dloss"] += 1
        else:
            retransmission_tracker[(tcp_seq, ip_dst, tcp_dst_port, ip_src, tcp_src_port)] = True
        ...
        flows[flow_id]["stcpb"] = pkt[TCP].seq if pkt[IP].src == ip_src else flows[flow_id].get("stcpb", 0)
        flows[flow_id]["dtcpb"] = pkt[TCP].seq if pkt[IP].src == ip_dst else flows[flow_id].get("dtcpb", 0)


        if len(flows[flow_id]["dst_timestamps"]) > 1:
            djitters = [flows[flow_id]["dst_timestamps"][i] - flows[flow_id]["dst_timestamps"][i-1] for i in range(1, len(flows[flow_id]["dst_timestamps"]))]
            flows[flow_id]["djit"] = statistics.stdev(djitters) if len(djitters) > 1 else 0
        else:
            flows[flow_id]["djit"] = 0

        if pkt.haslayer(HTTPRequest):
            flows[flow_id]["trans_depth"] = flows[flow_id].get("trans_depth", 0) + 1
            flows[flow_id]["ct_flw_http_mthd"] = flows[flow_id].get("ct_flw_http_mthd", 0) + 1

        if pkt.haslayer(HTTPResponse):
            if b"Content-Length" in bytes(pkt[HTTPResponse].fields.get("Headers", b"")):
                # Estimate response body length
                try:
                    headers = bytes(pkt[HTTPResponse].fields.get("Headers", b"")).decode(errors='ignore')
                    for line in headers.split("\r\n"):
                        if "Content-Length" in line:
                            flows[flow_id]["res_bdy_len"] = int(line.split(":")[1].strip())
                except:
                    flows[flow_id]["res_bdy_len"] = 0
        if pkt.haslayer(TCP) and (tcp_src_port == 21 or tcp_dst_port == 21):
            raw_load = bytes(pkt[TCP].payload).decode(errors='ignore')
            if "USER" in raw_load or "PASS" in raw_load:
                flows[flow_id]["is_ftp_login"] = 1
            if any(cmd in raw_load for cmd in ["LIST", "STOR", "RETR", "CWD", "PWD"]):
                flows[flow_id]["ct_ftp_cmd"] = flows[flow_id].get("ct_ftp_cmd", 0) + 1


        # Determine service based on destination port
        common_services = {80: "http", 21: "ftp", 25: "smtp", 22: "ssh", 53: "dns", 20: "ftp-data", 6667: "irc"}
        service = common_services.get(tcp_dst_port, "-")


        last_100_flows = list(flows.keys())[-100:]
        ct_dst_ltm = sum(1 for fid in last_100_flows if fid[2] == ip_dst)
        ct_src_ltm = sum(1 for fid in last_100_flows if fid[0] == ip_src)
        ...
        packet_info = {
            ...
            "stcpb": flows[flow_id]["stcpb"],
            "dtcpb": flows[flow_id]["dtcpb"],
            "djit": flows[flow_id]["djit"],
            "trans_depth": flows[flow_id].get("trans_depth", 0),
            "res_bdy_len": flows[flow_id].get("res_bdy_len", 0),
            "ct_flw_http_mthd": flows[flow_id].get("ct_flw_http_mthd", 0),
            "is_ftp_login": flows[flow_id].get("is_ftp_login", 0),
            "ct_ftp_cmd": flows[flow_id].get("ct_ftp_cmd", 0),
            "sloss": flows[flow_id]["sloss"],
            "dloss": flows[flow_id]["dloss"],
            "service": service,
...
df = pd.read_csv('/home/raga/extracted_packet_data.csv')

# Count of flows from same source IP to same dest port
df['ct_srv_src'] = df.groupby(['srcip', 'dsport'])['dsport'].transform('count')

# Count of flows to same destination IP and port
df['ct_srv_dst'] = df.groupby(['dstip', 'dsport'])['dsport'].transform('count')

# Count of flows with same source IP and destination port
df['ct_src_dport_ltm'] = df.groupby(['srcip', 'dsport'])['dsport'].transform('count')

# Count of flows with same destination IP and source port
df['ct_dst_sport_ltm'] = df.groupby(['dstip', 'sport'])['sport'].transform('count')

# Count of flows with same destination and source IP
df['ct_dst_src_ltm'] = df.groupby(['dstip', 'srcip'])['srcip'].transform('count')

# Save again if needed
df.to_csv('/home/raga/extracted_packet_data_with_counts.csv', index=False)
